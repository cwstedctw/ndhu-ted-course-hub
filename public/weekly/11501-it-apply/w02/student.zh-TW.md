...
回到私有櫃，打開 `w01-task-card.md`，用鉛筆按鈕編輯一行分類理由。按 Preview 看結果，再 Commit changes，寫清楚你改了什麼。提交後點檔案的最新 commit 訊息，觀察前後差異（diff）。有刪除符號 `−` 的行是這次移除的內容，新增符號 `+` 的行是加入的內容。
...
先確認正常版本已經 commit。只改替代材料裡的一行，例如把「雨天到圖書館」改成「雨天到操場」，提交訊息標明「練習：故意改壞雨天安排」。
...
4. 再提交一筆，例如「救回雨天安排：恢復圖書館版本」。最後重新打開 main 的檔案，核對那一行真的回來。
