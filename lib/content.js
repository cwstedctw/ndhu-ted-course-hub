...
/**
 * 場刊卡的「顯示態」（v3 W3-1 演講牆）。資料上只有 confirmed／tba／done 三種 status，
 * 但卡面要分四種樣子，所以這裡做一層衍生判斷——**只算給畫面看，不改 talks.json 一個字**。
 *
 *   'done'      status 已經是 done（Ted 或 pull 腳本回填過）
 *   'ended'     status 還是 confirmed，但 Asia/Taipei 的現在時刻已過該場當天的 11:30
 *               （F1 拍板 2026-07-25：**演講結束時間＝11:30**。talks.json 沒有結束時間
 *                 這個欄位，所以 11:30 只拿來當界線、卡面一律不印它）
 *   'confirmed' 講者已確認、還沒過 11:30
 *   'tba'       講者洽談中（含 talk 物件本身壞掉時的保底值——寧可少講，不腦補）
 *
 * ⚠️ 「下一場」（'next'）不由本函式回傳：那是「12 場裡的唯一一場」、要看全體才判得出來。
 *    呼叫端拿 getNextTalk() 挑出來那筆比對 id 自行升級（見 components/course/TalksWall.js）。
 *
 * 時區做法與 getNextTalk 相同：先把「現在」換算成 Asia/Taipei 牆鐘字串再比，
 * 否則靜態建置機器（常是 UTC）會把整場提早 8 小時判成已結束。
 *
 * @param {object} talk talks.json 的單筆 talk 物件
 * @param {Date} [now] 測試用可指定「現在時刻」；預設 new Date()
 * @returns {'done'|'ended'|'confirmed'|'tba'}
 */
export function getTalkDisplayStatus(talk, now = new Date()) {
  if (!talk || typeof talk !== 'object') return 'tba';
  if (talk.status === 'done') return 'done';
  if (talk.status !== 'confirmed') return 'tba';
  if (typeof talk.date !== 'string' || talk.date.trim() === '') return 'confirmed';
  ...
  const nowTaipei = new Date(taipeiWallClock.replace(' ', 'T'));
  const endsAt = new Date(`${talk.date.trim()}T11:30:00`); // F1：11:30 為界
  if (Number.isNaN(endsAt.getTime())) return 'confirmed'; // 日期字串壞掉＝不敢說它結束了

  // 界線本身（剛好 11:30）算已結束，與 getNextTalk 的 `now < 結束線` 互補、不會兩邊都收
  return nowTaipei.getTime() >= endsAt.getTime() ? 'ended' : 'confirmed';
}
